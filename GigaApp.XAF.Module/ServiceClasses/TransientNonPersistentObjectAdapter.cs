...
class TransientNonPersistentObjectAdapter
{
    ...
    private void ObjectSpace_CustomCommitChanges(object sender, HandledEventArgs e)
    {
        ...
        var toInsert = new List<NonPersistentObjectBase>();
        var toUpdate = new List<NonPersistentObjectBase>();
        foreach (var obj in toSave)
        {
            if (objectSpace.IsNewObject(obj))
            {
                toInsert.Add((NonPersistentObjectBase)obj);
            }
            else
            {
                toUpdate.Add((NonPersistentObjectBase)obj);
            }
        }
        var toDelete = objectSpace.GetObjectsToDelete(false);
        var toDelete2 = new List<NonPersistentObjectBase>();
        foreach (var obj in toDelete)
        {
            toDelete2.Add((NonPersistentObjectBase)obj);
        }
        if (toInsert.Count != 0 || toUpdate.Count != 0 || toDelete.Count != 0)
        {
            storage.SaveObjects(toInsert, toUpdate, toDelete2);
        }
        //e.Handled = false;// !!!
    }
